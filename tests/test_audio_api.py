...
# File: test_audio_api.cpython-312-pytest-9.0.3.pyc (Python 3.12)
...
audio_client = (lambda assistant, data_dir, monkeypatch: serveraudiofrom fastapi.testclient import TestClientimport jarvis.modules.audio, modulesrec = data_dir / 'audio' / 'recordings'gen = data_dir / 'audio' / 'generated'edited = data_dir / 'audio' / 'edited'for d in (rec, gen, edited):
d.mkdir(parents = True, exist_ok = True)lib = {
'recordings': rec,
'generated': gen,
'edited': edited }monkeypatch.setattr(audio_mod, 'AUDIO_DIR', data_dir / 'audio')monkeypatch.setattr(audio_mod, 'RECORDINGS_DIR', rec)monkeypatch.setattr(audio_mod, 'GENERATED_DIR', gen)monkeypatch.setattr(audio_mod, 'EDITED_DIR', edited)monkeypatch.setattr(audio_mod, 'AUDIO_LIBRARY_DIRS', lib)monkeypatch.setattr(audio_mod, 'DATA_DIR', data_dir)monkeypatch.setattr('jarvis.config.DATA_DIR', data_dir)monkeypatch.setattr(gui_server, 'assistant', assistant)monkeypatch.setattr(gui_server, 'DATA_DIR', data_dir)monkeypatch.setattr('jarvis.gui.extra_routes.DATA_DIR', data_dir)monkeypatch.setattr('jarvis.auth.api_key_enabled', (lambda : False))
...
def test_audio_session_roundtrip(audio_client):
    res = audio_client.post('/api/audio/session', data = {
        'transcript': 'hello',
    ...
    @py_assert0 = sess['session']['transcript']
    @py_assert3 = 'hello'
...
def test_audio_delete_file(audio_client, data_dir):
    ...
    res = audio_client.post('/api/audio/delete', data = {
        'path': str(wav),
        'category': 'recordings' })
    @py_assert1 = res.status_code
    @py_assert4 = 200
    ...
    @py_assert2 = None
    @py_assert3 = None
    @py_assert1 = wav.exists
    @py_assert3 = @py_assert1()
    @py_assert5 = not @py_assert3
    if not @py_assert5:
        @py_format6 = 'assert not %(py4)s\n{%(py4)s = %(py2)s\n{%(py2)s = %(py0)s.exists\n}()\n}' % {
            'py0': @pytest_ar._saferepr(wav) if 'wav' in @py_builtins.locals() or @pytest_ar._should_repr_global_name(wav) else 'wav',
            'py2': @pytest_ar._saferepr(@py_assert1),
            'py4': @pytest_ar._saferepr(@py_assert3) }
        raise AssertionError(@pytest_ar._format_explanation(@py_format6))
    @py_assert1 = None
...
def test_audio_file_rejects_outside_library(audio_client, data_dir):
    ...
    res = audio_client.get('/api/audio/file', params = {
        'path': str(secret) })
    @py_assert1 = res.status_code
    @py_assert4 = 404
    @py_assert3 = @py_assert1 == @py_assert4
...
def test_audio_whisper_language(audio_client):
    res = audio_client.post('/api/audio/whisper-language', data = {
        'language': 'en' })
    @py_assert1 = res.status_code
    @py_assert4 = 200
    @py_assert3 = @py_assert1 == @py_assert4
    ...
    @py_assert3 = None
    @py_assert4 = None
    @py_assert0 = res.json()['whisper_language']
    @py_assert3 = 'en'
    @py_assert2 = @py_assert0 == @py_assert3
    if not @py_assert2:
        @py_format5 = @pytest_ar._call_reprcompare(('==',), (@py_assert2,), ('%(py1)s == %(py4)s',), (@py_assert0, @py_assert3)) % {
...
def test_audio_diarize_mock(audio_client, assistant, monkeypatch, data_dir):
    ...
    wav.write_bytes(b'x')
    monkeypatch.setattr(assistant.audio, 'diarize', (lambda path, num_speakers = (None,): {
'ok': True,
'engine': 'test',
'segments': [
{
...
'text': 'hi' }],
'transcript': '**A:** hi' }))
    res = audio_client.post('/api/audio/diarize', data = {
        'path': str(wav) })
    @py_assert1 = res.status_code
    @py_assert4 = 200
    @py_assert3 = @py_assert1 == @py_assert4
...
def test_audio_piper_speed(audio_client):
    res = audio_client.post('/api/audio/piper-speed', data = {
        'speed': '1.1' })
    @py_assert1 = res.status_code
    @py_assert4 = 200
    @py_assert3 = @py_assert1 == @py_assert4
    ...
    @py_assert3 = None
    @py_assert4 = None
    @py_assert0 = res.json()['piper_speed']
    @py_assert3 = 1.1
    @py_assert2 = @py_assert0 == @py_assert3
    if not @py_assert2:
        @py_format5 = @pytest_ar._call_reprcompare(('==',), (@py_assert2,), ('%(py1)s == %(py4)s',), (@py_assert0, @py_assert3)) % {
...
def test_audio_transcribe_mock(audio_client, assistant, monkeypatch, data_dir):
    wav = data_dir / 'audio' / 'recordings' / 'sample.wav'
    wav.parent.mkdir(parents = True, exist_ok = True)
    wav.write_bytes(b'RIFF')
    monkeypatch.setattr(assistant.audio, 'transcribe', (lambda path: 'hello world'))
    res = audio_client.post('/api/audio/transcribe', data = {
        'path': str(wav) })
    @py_assert1 = res.status_code
    @py_assert4 = 200
    @py_assert3 = @py_assert1 == @py_assert4
    ...
    @py_assert3 = None
    @py_assert4 = None
    @py_assert0 = res.json()['transcript']
    @py_assert3 = 'hello world'
    @py_assert2 = @py_assert0 == @py_assert3
    if not @py_assert2:
        @py_format5 = @pytest_ar._call_reprcompare(('==',), (@py_assert2,), ('%(py1)s == %(py4)s',), (@py_assert0, @py_assert3)) % {
            'py1': @pytest_ar._saferepr(@py_assert0),
...
def test_audio_music_async_job(audio_client, monkeypatch):
    monkeypatch.setattr('jarvis.music_gen.generate_music', (lambda prompt, duration = (10,): '/tmp/song.wav'))
    res = audio_client.post('/api/audio/music', data = {
        'prompt': 'calm piano',
        'duration': '8',
        'async_job': '1' })
    @py_assert1 = res.status_code
