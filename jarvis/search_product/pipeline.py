...
def run_search(
    ...
    *,
    ...
    if do_parallel and len(corpora) > 1:
        with ThreadPoolExecutor(max_workers=min(8, len(corpora))) as pool:
            futs = {pool.submit(_one, c): c for c in corpora}
            for fut in as_completed(futs, timeout=12):
                try:
                    corp, hits, err = fut.result(timeout=8)
                except Exception as exc:
                    corp = futs[fut]
...
def format_search_message(result: dict[str, Any]) -> str:
    ...
    q = result.get("query", "")
    if not hits:
        return f"No matches for **{q}** across federated sources."
    lines = [
        f"**Search:** _{q}_",
