"""Fly Tying history — sessions, suggestions, vision scans (shared censored/uncensored storage)."""
...
HISTORY_FILE = DATA_DIR / "flytying_product" / "history.jsonl"
MAX_ENTRIES = 500
...
def add_entry(entry: dict[str, Any]) -> dict[str, Any]:
    row = {
        ...
        "kind": entry.get("kind") or "event",  # session | suggestion | vision_scan | note
    ...
    try:
        for line in HISTORY_FILE.read_text(encoding="utf-8").splitlines():
            ...
            if q:
                blob = " ".join(
                    str(row.get(k) or "")
                    for k in ("summary", "detail", "recipe_id", "recipe_name", "kind", "path", "source")
                ).lower()
                if q not in blob:
                    continue
            rows.append(row)
...
def presentation_for_profile(
    ...
    *,
    ...
    Never regenerate or delete original analysis / notes.
    """
    out = dict(entry)
