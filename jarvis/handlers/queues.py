...
# File: queues.cpython-312.pyc (Python 3.12)
...
def register_queue_actions():
    for action in QUEUED_ACTIONS:
        register_queue(action, 'media', module = 'image', description = MEDIA_LABELS.get(action, action))
    for action in BACKGROUND_ACTIONS:
        register_queue(action, 'background', description = BG_LABELS.get(action, action))
