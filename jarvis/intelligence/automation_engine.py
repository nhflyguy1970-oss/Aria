...
from jarvis.automation.execution import FAILED, SKIPPED, SUCCEEDED, normalize_result
from jarvis.automation.paths import LEGACY_RULES_FILE, RULES_FILE, ensure_dirs
from jarvis.automation.registry import get_action, validate_action

log = logging.getLogger("jarvis.intelligence.automation")

# Prefer product rules file; fall back to legacy
...
def _save() -> None:
    ...
    try:
        LEGACY_RULES_FILE.write_text(json.dumps({"rules": payload["rules"]}, indent=2), encoding="utf-8")
    except Exception:
        pass
...
def start_engine(runner: Callable[[AutomationRule], dict[str, Any]] | None = None) -> dict[str, Any]:
